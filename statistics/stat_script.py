...
import threading
import os
from concurrent.futures import ProcessPoolExecutor, ALL_COMPLETED, ThreadPoolExecutor
...
def count_skills(m_df, num_workers=6):
    ...
    # def worker_job(arguments):
    #     group, year, m_result_dict = arguments
    #     pid = os.getpid()
    #     thread = threading.current_thread()
    #     worker_prefix = f"[COUNTING][GROUPING] WORKER ({thread.name} of process with id:{pid}) "
    #     print(f"{worker_prefix} STARTS with {year}")
    ...
    poolexec = ThreadPoolExecutor(max_workers=num_workers)
    ...
    with Manager() as manager:
        ...
        with poolexec as exec:
            ...
            #exec.map(worker_job, [(group, yr, shared_dict) for yr, group in groups])
...
def main():
    ...
    skills_for_vac = ['1с разработчик', "1c разработчик", '1c', '1с', '1 c', '1 с']
    csv = 'vacancies.csv'
