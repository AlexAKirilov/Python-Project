...
def get_top20_vac():
    keywords = ['1с разработчик', "1c разработчик", '1c', '1с', '1 c', '1 с']
    url = "https://api.hh.ru/vacancies"
    today = datetime.now().replace(hour=0, minute=0, second=0)
    today = datetime.now() - timedelta(hours=24)
    params = {
        "text": f"name:(({' OR '.join(keywords)}) AND (Разработчик OR Программист OR developer)) AND description:(({' OR '.join(keywords)}) AND NOT JavaScript)",
